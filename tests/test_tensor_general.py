...
if numba.cuda.is_available():
    ...
    def test_mul_practice5():
        ...
        size = 33
        x = [[[random.random() for i in range(size)] for j in range(size)] for _ in range(2)]
        y = [[[random.random() for i in range(size)] for j in range(size)] for _ in range(2)]
        z = minitorch.tensor(x, backend=shared["fast"]) @ minitorch.tensor(
            y, backend=shared["fast"]
        )
    ...
    def test_mul_practice6():
        ...
        size_b = 40
        size_in = 33
        x = [[[random.random() for i in range(size_in)] for j in range(size_a)] for _ in range(2)]
        y = [[[random.random() for i in range(size_b)] for j in range(size_in)] for _ in range(2)]
        z = minitorch.tensor(x, backend=shared["fast"]) @ minitorch.tensor(
            y, backend=shared["fast"]
        )
